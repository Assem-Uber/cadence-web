...
import {
  ...
  LOGOUT_ITEM,
  OIDC_LOGIN_ITEM,
} from './app-nav-bar.constants';
import { cssStyles, overrides } from './app-nav-bar.styles';
import { type UserMenuItem } from './app-nav-bar.types';
import formatSessionExpiryLabel from './helpers/format-session-expiry-label';
...
export default function AppNavBar() {
  ...
  const handleLogout = useCallback(
    async (trigger: 'manual' | 'expired') => {
      ...
      if (isOidcAuth) {
        ...
        logoutWithRedirect(
          trigger === 'manual' ? 'signed-out' : 'session-expired'
        );
        return;
      }
      ...
      if (isJwtAuth) {
        ...
        logoutWithRedirect(
          trigger === 'manual' ? 'signed-out' : 'session-expired',
          trigger === 'manual' ? '/domains' : currentReturnTo
        );
        return;
      }
  ...
  const handleUserMenuSelect = useCallback(
    (item: { info?: unknown; label?: string }) => {
      const itemInfo = typeof item.info === 'string' ? item.info : undefined;
      if (itemInfo === LOGIN_ITEM || itemInfo === OIDC_LOGIN_ITEM) {
        login(currentReturnTo);
      } else if (
        itemInfo === LOGOUT_ITEM ||
        item.label === 'Sign out' ||
        item.label === 'Logout'
      ) {
    ...
    [handleLogout, currentReturnTo, login]
  );

  useEffect(() => {
    const authNotice = searchParams.get('authNotice');
    if (!authNotice || !isOidcAuth) {
      return;
    }

    const clearAuthNoticeFromUrl = () => {
  ...
  useEffect(() => {
    if (!isAuthEnabled || isAuthLoading || isOidcAuth || !isJwtAuth) return;
    ...
    if (prevIsValidToken === true && !isValidToken) {
      ...
      logoutWithRedirect('session-expired', currentReturnTo);
    }
  }, [
    currentReturnTo,
    isValidToken,
  ...
  useEffect(() => {
    ...
    const logoutIfExpiryMatches = async () => {
      if (logoutInFlightRef.current) return;
      if (latestExpiresAtRef.current !== expiresAtMs) return;

      // Strategies that support session recovery (e.g. OIDC refresh tokens)
      // get a silent recovery attempt before logout; unsupported strategies
      // resolve to 'noop' immediately. On success the refetched expiry
      // re-arms this timer; on 'redirect' recovery has already navigated.
      const recovery = await recoverSession(currentReturnTo);
  ...
  useEffect(() => {
    if (!isOidcAuth || !isValidToken || expiresAtMs === undefined) {
      return;
    }
    ...
    const intervalMs = remainingMs < 60_000 ? 1000 : 30_000;
    const intervalId = window.setInterval(
      () => setNowMs(Date.now()),
      intervalMs
    );
    return () => window.clearInterval(intervalId);
  }, [isOidcAuth, isValidToken, expiresAtMs]);

  useEffect(() => {
    // No warning when the session can be refreshed silently — expiry will be
    // extended without signing the user out.
    if (
      !isOidcAuth ||
      !isValidToken ||
      expiresAtMs === undefined ||
  ...
  }, [nowMs, expiresAtMs, isOidcAuth, isValidToken, canRefreshSession, enqueue]);
  ...
  const userItems = useMemo<UserMenuItem[] | undefined>(() => {
    ...
    if (isJwtAuth) {
      ...
      return [{ label: 'Logout', info: LOGOUT_ITEM }];
    }
    if (isOidcAuth) {
      if (!isValidToken) {
        return [{ label: 'Sign in', info: OIDC_LOGIN_ITEM }];
      }
      return [{ label: 'Sign out', info: LOGOUT_ITEM }];
    }
    return undefined;
  }, [isAuthEnabled, isJwtAuth, isOidcAuth, isValidToken]);
  ...
  const username = useMemo(() => {
    ...
    return isValidToken
      ...
      : isOidcAuth
        ? 'Sign in'
        : 'Authenticate';
  }, [isAuthEnabled, isAuthLoading, isValidToken, isOidcAuth, userName]);

  const usernameSubtitle = useMemo(() => {
    if (!isAuthEnabled || !isValidToken) {
      return isAuthEnabled && isJwtAuth ? 'Sign in required' : undefined;
    }

    if (isOidcAuth && expiresAtMs !== undefined) {
      return formatSessionExpiryLabel(expiresAtMs, nowMs);
    }
    return undefined;
  }, [isAuthEnabled, isValidToken, isJwtAuth, isOidcAuth, expiresAtMs, nowMs]);

  const mapItemToNode = useCallback(
    (item: { label: string; info?: unknown }) => item.label,
    []
  );
  ...
  return (
    <BaseAppNavBar
      ...
      userImgUrl={isValidToken ? pictureUrl : undefined}
      userItems={userItems}
      onUserItemSelect={handleUserMenuSelect}
    />
  );
}
